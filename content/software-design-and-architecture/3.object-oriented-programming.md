...
In this example, we define a `Counter` function that has a private variable count and a private method `increment`. We then return an object that has a single method `increment`, which can be used to `increment` the counter.

When we call `Counter()`, it returns an object with a single method `increment`. This method has access to the private variable count and the private method `increment`, even though they are not directly accessible from outside the function.

This is an example of encapsulation because we have hidden the implementation details (the private variable and method) from the outside world and only exposed a public interface (the `increment` method). This makes the code more modular, easier to understand, and less prone to bugs.
...
In this example, the `count` variable is hidden from the outside world and can only be accessed through the increment function. The increment function is returned as a public method from the `Counter` function and can be called from outside. The value of the `count` variable is incremented each time the increment method is called, and the updated value is logged to the console.
...
In this example, `Animal` is the base class, and `Dog` is the derived class.   inherits the name and age properties, and the eat() method from `Animal`.   also defines its own bark() method, which is specific to dogs. We create an instance of the `Dog` class named `myDog`, and call its inherited `eat()` method and its own `bark()` method.
...
- https://www.linkedin.com/learning/typescript-object-oriented-programming/what-s-the-oop-in-typescript-all-about-2
